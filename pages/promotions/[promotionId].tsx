  ...
  const handleDeleteSelected = () => {
    if (window.confirm('Are you sure you want to delete the selected codes?')) {
      ...
      const deletionPromises = selectedCodes.map(codeId => {
        return fetch(`/api/promotions/${promotionId}/codes?codeId=${codeIds}&context=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..4MVkZez-Fr4bhOsw14rc0fCb9-FVj3M8qUYiDlqCUKY`,
                     { method: 'DELETE'})
          .then(response => {
            if (!response.ok) {
